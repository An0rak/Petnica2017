...
void drawEpipolarLines(cv::Mat& F, cv::Mat& img1, cv::Mat& img2, std::vector<cv::Point2f> points1, std::vector<cv::Point2f> points2, float inlierDistance = -1)
{
	...
	for(size_t i=0; i<points1.size(); i++)
	{
		...
		bool DecomposeEtoRandT(const Mat_<float> E,Mat_<float>& R1,Mat_<float>& R2,Mat_<float>& t1,Mat_<float>& t2)
		{
			...
			if(singular_values_ratio > 1) singular_values_ratio = 1 / singular_values_ratio;
			...
			Matx33f W(0,-1,0,
				...
				bool triangulateAndCheckReproj(const Mat R, const Mat t, vector<Point2f> correctMatchingPoints1,vector<Point2f> correctMatchingPoints2,
					Mat cameraMatrix, vector<Point3f>& points3dfirst, vector<Point3f>& points3dsecond) {
						...
						FileStorage fw("Core.yml", FileStorage::WRITE);
						...
						projectPoints(pt_3d,rvec,tvec,cameraMatrix,Mat(),reprojected_pt_set1);
						//    cout << Mat(reprojected_pt_set1).rowRange(0,10) << endl;
						vector<Point2f> bootstrapPts_v = Mat(bootstrap_kp);
						...
						if(reprojErr < 5)
						{
							...
							cout << "keeping " << points3dfirst.size() << " nicely reprojected points \n";
							return true;
						}
						return false;
					}
					...
					vector<float> getPlane(Mat img1, Mat img2, Mat cameraMatrixread, Mat distortionCoeffread, vector<Point2f>& correctMatchingPoints1, vector<Point2f>& correctMatchingPoints2,
						vector<KeyPoint>& correctKeyPoints1, vector<KeyPoint>& correctKeyPoints2,vector<DMatch>& correctMatches, vector<Point3f>& points3dfirst, vector<Point3f>& points3dsecond )
						{
							...
							fw << "Findamental" << fundamentalMatrix;
							...
							int k= 100;
							plane = ransac(points3dfirst, k, 2, 3*datasetSize/4);
							return plane;
						}
